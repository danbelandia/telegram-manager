# Delta for publications — Slice 2 (Foto URL + Botones + Multi-Grupo + Filtro)

> Change: `publications` (Slice 2 de 3). Modifica la spec canónica
> `openspec/specs/publications/spec.md` (archivada de slice 1) para
> extender publicaciones con foto por URL, botones inline de URL,
> envío multi-grupo secuencial y filtro por grupo en el historial.
> Respeta bugfix #172 (`permissionOk` usa `bot_status == administrator`,
> NO reintroducir checks sobre claves `can_*`).
...
(Previously: la tabla se creaba en slice 1 con `text`, `status`,
...
(Previously: la firma era `SendMessage(ctx, chatID, text,
disableWebPagePreview)` sin keyboard. Slice 1 archivada no enviaba
botones.)
...
(Previously: el body era `{text, group_id}` con un único grupo y la
respuesta 201 contenía la fila única. Slice 1 no aceptaba
...
(Previously: el endpoint listaba las 50 más recientes sin filtro,
sin incluir `photo_url` ni `buttons` en la respuesta. Slice 2
...
(Previously: el form tenía solo textarea + select de grupo único y
el listado mostraba texto/grupo/status/fecha. Slice 2 agrega foto,
editor de botones, multi-select, preview de foto y botones en el
...
(Previously: los tests cubrían SendMessage, servicio con
...
(Previously: los tests cubrían carga, error y creación con grupo
único. Slice 2 agrega casos con foto+botones+multi-grupo y filtro.)
...
(Previously: cada publicación generaba exactamente un log. Slice 2
mantiene un log por intento de publicación, pero como
