...
- [x] 3.7 E2E manual ⚠️ **ejecutado parcialmente** contra BBBTest (básico `-5534804488` + supergroup `-1004424813387`, bot admin, usuario Keiber `5268437131`): PIN ✅, DELETE ✅, LOCK ✅, UNLOCK ✅, BAN ✅ (básico y supergroup), MUTE ✅ (supergroup), UNMUTE ✅, UNBAN ✅ (supergroup). Fallos esperados por limitación de Bot API: MUTE/UNBAN en grupo básico → `TELEGRAM_ERROR` con motivo real (registrado en logs). **Discoveries**: (a) `unbanChatMember` y `restrictChatMember` solo aplican en supergroups (grupo básico: ban funciona, unban no) — documentar en referencia; (b) al convertir grupo a supergroup cambia el ID (pierde prefijo -100→gana -100...) y el bot/miembros quedan fuera: hay que re-agregarlos; (c) **fix aplicado**: `respondModerationError` mapeaba `ErrTelegramAPI` (400 no clasificado) a `INTERNAL_ERROR`; ahora `errors.As` → 502 `TELEGRAM_ERROR` (spec §18). approve/reject **NO probados manualmente** (requiere modo de aprobación en el grupo; usuario decidió saltar) — cubiertos por tests unitarios
- [x] 3.8 Suite completa `go test ./...` verde; gofmt/vet limpio (verificado antes del commit b153e72^..HEAD; +fix 502 tras E2E)
